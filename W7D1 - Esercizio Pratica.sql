...
#Esercizio1, Scrivi una query per verificare che il campo ProductKey nella tabella DimProduct sia una chiave primaria.
...
#Esercizio2 #Scrivi una query per verificare che la combinazione dei campi SalesOrderNumber e SalesOrderLineNumber sia una PK.
...
#Esercizio3 Conta il numero transazioni (SalesOrderLineNumber) realizzate ogni giorno a partire dal 1 Gennaio 2020

SELECT 
    DATE(FR.OrderDate) AS Giorno,
    COUNT(FR.SalesOrderLineNumber) AS NumeroTransazioni
FROM factresellersales as FR
WHERE FR.OrderDate >= '2020-01-01'
GROUP BY DATE(FR.OrderDate)
ORDER BY Giorno;

#Esercizio4 Calcola il fatturato totale (FactResellerSales.SalesAmount), la quantità totale venduta 
#(FactResellerSales.OrderQuantity) e il prezzo medio di vendita FactResellerSales.UnitPrice) per prodotto (DimProduct) 
...
#Esercizio5 Calcola il fatturato totale (FactResellerSales.SalesAmount) e la quantità totale venduta (FactResellerSales.OrderQuantity) 
...
#Esercizio6 Calcola il fatturato totale per area città (DimGeography.City) realizzato a partire dal 1 Gennaio 2020.
